...
{url:"/edit",templateUrl:"partials/battle_edit.html",controller:"listEditCtrl"}).state("filter",{url:"/filter",templateUrl:"partials/filter.html"}).state("backup",{url:"/backup",templateUrl:"partials/backup.html",controller:"backupCtrl"}).state("stats",{url:"/stats",templateUrl:"partials/stats.html",controller:"statsCtrl"})}]).config(["$compileProvider",function(a){a.urlSanitizationWhitelist(/^\s*(https?|ftp|mailto|blob):/)}]);angular.module("jlogApp.controllers").controller("backupCtrl",["$scope","backup",function(a,c){console.log("init backupCtrl");a.backup=c;a.readBackupFile=function(b){a.backup.read(b,function(b){a.newBattles(b);c.generate(a.battles);a.$apply("backup.read_result = 'loaded file'")},function(b){a.$apply("backup.read_result = '"+b+"'")})};c.generate(a.battles)}]);angular.module("jlogApp.controllers").controller("listCtrl",["$scope","$state","battle","selection",function(a,c,b,d){console.log("init listCtrl");a.show_list=!0;a.selection=d;a.addBattle=function(){a.battle_index=a.battles.length;a.battle=b.create();c.go("battle.edit")};a.viewBattle=function(b){a.battle_index=b;a.battle=a.battles[b];c.go("battle.view")};a.close=function(){a.show_list=!0;c.go("battle.list")};a.selectionRemove=function(){a.selection.remove(a.battles)&&a.updateBattles()};a.selectionSet=
...
function(a){d.update(a)},!0);a.selection.reset(a.battles)}]);angular.module("jlogApp.controllers").controller("listEditCtrl",["$scope","$state","$window","battle","opponents","events","scenarios","tags",function(a,c,b,d,k,e,p,n){console.log("init listEditCtrl "+a.battle_index);console.log(a.battle);void 0===a.battle&&c.go("battle.list");a.saveBattle=function(){a.battles.length>a.battle_index?a.battles[a.battle_index]=a.battle:a.battles.push(a.battle);a.updateBattles();a.close()};a.addOpponent=function(){var e=b.prompt("Enter new opponent name :"),e=null!=e?
e.trim().toLowerCase():"";0<e.length&&(a.opponents.push(e),a.opponents.sort(),k.store(a.opponents),a.battle.opponent.name=e)};a.addEvent=function(){var c=b.prompt("Enter new event name :"),c=null!=c?c.trim():"";0<c.length&&(a.events.push(c),a.events.sort(),e.store(a.events),a.battle.setup.event=c)};a.addScenario=function(){var e=b.prompt("Enter new scenario name :"),e=null!=e?e.trim():"",c=e.toLowerCase();0<e.length&&(a.scenarios[c]={name:e},p.store(a.scenarios),a.battle.setup.scenario=c)};a.addTag=
function(){var e=b.prompt("Enter new tag name :"),e=null!=e?e.trim():"";0<e.length&&(a.tags.push(e),a.tags.sort(),n.store(a.tags),a.battle.tags=angular.isArray(a.battle.tags)?a.battle.tags:[],a.battle.tags.push(e),a.battle.tags.sort())};var l=function(b,e,c){b=a[e].indexOf(b);0>b||(a[e].splice(b,1),c.store(a[e]))},q={opp:{get:function(a){return a.opponent.name},set:function(a,b){a.opponent.name=b},remove_from_list:function(a){l(a,"opponents",k)}},event:{list:"event",service:e,get:function(a){return a.setup.event},
set:function(a,b){a.setup.event=b},remove_from_list:function(a){l(a,"events",e)}},scenario:{get:function(a){return a.setup.scenario},set:function(a,b){a.setup.scenario=b},remove_from_list:function(b){a.scenarios.hasOwnProperty(b)&&(delete a.scenarios[b],p.store(a.scenarios))}}};a.del=function(e){if(q.hasOwnProperty(e)){var c=q[e].get,p=q[e].set,g=c(a.battle),g=null!=g?g:"";if(!(0>=g.length)&&b.confirm('Forget everything about "'+g+'" ?')){p(a.battle,null);var d;for(d=0;d<a.battles.length;d++)g===
c(a.battles[d])&&p(a.battles[d],null);a.updateBattles();q[e].remove_from_list(g)}}};a.delTag=function(){var e=a.battle.tags;if(angular.isArray(e)&&!(0>=e.length)){var c="Forget everything about these tags ?\r\n",p;for(p=0;p<e.length;p++)c+="\t"+e[p]+"\r\n";if(b.confirm(c)){a.battle.tags=[];var g,d,k;for(p=0;p<e.length;p++)for(d=e[p],g=0;g<a.battles.length;g++)if(k=a.battles[g],angular.isArray(k.tags))for(;0<=(c=k.tags.indexOf(d));)k.tags.splice(c,1);a.updateBattles();for(p=0;p<e.length;p++)for(;0<=
(c=a.tags.indexOf(e[p]));)a.tags.splice(c,1)}}};a.battle=angular.copy(a.battle)}]);angular.module("jlogApp.controllers").controller("listViewCtrl",["$scope","$state",function(a,c){console.log("init listViewCtrl "+a.battle_index);console.log(a.battle);void 0===a.battle&&c.go("battle.list");a.editBattle=function(){c.go("battle.edit")};a.deleteBattle=function(){window.confirm("You sure you wanna delete this battle ?")&&(a.battles.splice(a.battle_index,1),a.updateBattles(),a.close())}}]);angular.module("jlogApp.controllers").controller("mainCtrl",["$scope","factions","opponents","events","scenarios","tags","scores","battles","battle_sort","filter","stats","battle_list_display",function(a,c,b,d,k,e,p,n,l,q,h,f){console.log("init mainCtrl");a.collapse_navbar=!0;a.factions=c;a.scores=p;a.filter_active=!1;a.filter_invert=!1;a.filter=q.init();a.stats=h;a.list_display=f;a.sort=l();var s=function(){a.filter.clearCache();a.list_display.reset(a.battles,a.filter,a.filter_active,a.filter_invert,
a.sort);a.stats.reset()};a.updateBattles=function(){n.update(a.battles);s()};a.newBattles=function(e){a.battles=n.create(e);a.opponents=b.create(a.battles);a.events=d.create(a.battles);a.scenarios=k.create(a.battles)};a.battles=n.init();a.opponents=b.init(a.battles);a.events=d.init(a.battles);a.scenarios=k.init(a.battles);a.tags=e.init(a.battles);s();a.$watch("filter",function(){a.filter.clearCache();a.stats.filtered.reset();a.filter.update()},!0);a.$watch("filter_invert",function(){a.stats.filtered.reset()});
